...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or moneta.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateMonetas(fGenerate, pwalletMain, nGenProcLimit);
    }
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "PerfectCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "PerfectCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PerfectCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PerfectCoin is downloading blocks...");
