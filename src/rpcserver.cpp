...
std::string CRPCTable::help(std::string strCommand) const
{
    ...
    for (std::map<std::string, const CRPCCommand*>::const_iterator mi = mapCommands.begin(); mi != mapCommands.end(); ++mi)
    {
        ...
        if (fAllAnon)
        {
            if (strMethod != "sendperfectcointoanon"
                && strMethod != "sendanontoanon"
                && strMethod != "sendanontoperfectcoin"
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "Stop PerfectCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "PerfectCoin server stopping";
}
...
{ //  name                      actor (function)         okSafeMode threadSafe reqWallet
  //  ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "sendperfectcointoanon",          &sendperfectcointoanon,          false,     false,     false },
    { "sendanontoanon",         &sendanontoanon,         false,     false,     false },
    { "sendanontoperfectcoin",          &sendanontoperfectcoin,          false,     false,     false },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        std::string strWhatAmI = "To use perfectcoind";
