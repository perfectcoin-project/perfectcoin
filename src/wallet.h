...
public:
    ...
    int64_t GetPerfectCoinXBalance() const;
    ...
    bool SendPerfectCoinToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToPerfectCoin(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetPerfectCoinXDebit(const CTxIn& txin) const;
    int64_t GetPerfectCoinXCredit(const CTxOut& txout) const;
    ...
    int64_t GetDebit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txin.IsAnonInput())
            {
                nDebit += GetPerfectCoinXDebit(txin);
            } else
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetPerfectCoinXCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nPC, int64_t& nPerfectCoinX) const
    {
        nPC = 0;
        nPerfectCoinX = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nPerfectCoinX += GetPerfectCoinXCredit(txout);
            } else
                nPC += GetCredit(txout);
            if (!MoneyRange(nPC)
                || !MoneyRange(nPerfectCoinX))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailablePerfectCoinXCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailablePerfectCoinXCreditCached;
    
    mutable int64_t nCredPCCached;
    mutable int64_t nCredPerfectCoinXCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailablePerfectCoinXCreditCached = false;
        
        nCredPCCached = 0;
        nCredPerfectCoinXCached = 0;
        ...
        nAvailablePerfectCoinXCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredPC, int64_t& nCredPerfectCoinX, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredPC = nCredPerfectCoinX = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredPCCached = 0;
            nCredPerfectCoinXCached = 0;
            pwallet->GetCredit(*this, nCredPCCached, nCredPerfectCoinXCached);
            fCreditSplitCached = true;
        };
        
        nCredPC = nCredPCCached;
        nCredPerfectCoinX = nCredPerfectCoinXCached;
        return true;
    }
    ...
    int64_t GetAvailablePerfectCoinXCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailablePerfectCoinXCreditCached)
            return nAvailablePerfectCoinXCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailablePerfectCoinXCredit() : value out of range");
            };
        };
        
        nAvailablePerfectCoinXCreditCached = nCredit;
        fAvailablePerfectCoinXCreditCached = true;
        return nCredit;
    };
