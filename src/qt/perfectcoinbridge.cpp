...
#include "perfectcoinbridge.moc"

PerfectCoinBridge::PerfectCoinBridge(PerfectCoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
PerfectCoinBridge::~PerfectCoinBridge()
{
...
void PerfectCoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void PerfectCoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void PerfectCoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void PerfectCoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void PerfectCoinBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void PerfectCoinBridge::populateOptions()
{
...
// Transactions
bool PerfectCoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void PerfectCoinBridge::clearRecipients()
{
    recipients.clear();
}

bool PerfectCoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 PC, 1 PCX
        switch(rcp.txnTypeInd)
        {
            case TXT_PC_TO_PC:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_PC_TO_ANON:
                formatted.append(tr("<b>%1</b> to PCX %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to PCX %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_PC:
                formatted.append(tr("<b>%1</b>, ring size %2 to PC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsPerfectCoinXBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your PCX balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void PerfectCoinBridge::openAddressBook(bool sending)
{
...
void PerfectCoinBridge::openCoinControl()
{
...
void PerfectCoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void PerfectCoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap PerfectCoinBridge::listAnonOutputs()
{
...
void PerfectCoinBridge::populateTransactionTable()
{
...
void PerfectCoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void PerfectCoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString PerfectCoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void PerfectCoinBridge::populateAddressTable()
{
...
void PerfectCoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void PerfectCoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString PerfectCoinBridge::newAddress(bool own)
{
...
QString PerfectCoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void PerfectCoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool PerfectCoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool PerfectCoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void PerfectCoinBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void PerfectCoinBridge::appendMessage(int row) {
...
void PerfectCoinBridge::populateMessageTable()
{
...
void PerfectCoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool PerfectCoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool PerfectCoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString PerfectCoinBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool PerfectCoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool PerfectCoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void PerfectCoinBridge::connectSignals()
{
...
QVariantMap PerfectCoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap PerfectCoinBridge::listLatestBlocks()
{
...
QVariantMap PerfectCoinBridge::findBlock(QString searchID)
{
...
QVariantMap PerfectCoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap PerfectCoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap PerfectCoinBridge::txnDetails(QString blkHash, QString txnHash)
{
