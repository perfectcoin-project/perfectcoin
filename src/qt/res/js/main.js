...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uPC",
                this.display = "&micro;PC";
                break;

            case 3:
                this.name    = "sPC",
                this.display = "PerfectCoinoshi";
                break;

            default:
                this.name = this.display = "PC";
        }
...
var overviewPage = {
    init: function() {
        ...
        this.perfectcoinXBal = $("#perfectcoinXBal"),
        ...
        // Announcement feed
        $.ajax({
            url:"http://ajax.googleapis.com/ajax/services/feed/load?v=2.0&q=http://blog.perfectcoin.info/rss/atom",
            dataType: 'jsonp'
    ...
    updateBalance: function(balance, perfectcoinXBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            perfectcoinXBal   = this.perfectcoinXBal  .data("orig"),
            ...
            this.perfectcoinXBal  .data("orig", perfectcoinXBal),
        ...
        this.formatValue("perfectcoinXBal",   perfectcoinXBal);
    ...
    updateTransaction: function(txn) {
        var format = function(tx) {
            ...
            return "<a id='"+tx.id.substring(0,17)+"' title='"+tx.tt+"' class='transaction-overview' href='#' onclick='$(\"#navitems [href=#transactions]\").click();$(\"#"+tx.id+"\").click();'>\
                                                <span class='"+(tx.t == 'input' ? 'received' : (tx.t == 'output' ? 'sent' : (tx.t == 'inout' ? 'self' : 'stake')))+" icon no-padding blue-perfectcoin'>\
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. MXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter a PerfectCoin address (e.g. MXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="PerfectCoin"                    ' + (unit.type == 0 ? "selected" : "") + '>PC</option> \
                    <option value="1" title="Milli-PerfectCoin (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mPC</option> \
                    <option value="2" title="Micro-PerfectCoin (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;PC</option> \
                    <option value="3" title="PerfectCoinoshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>PerfectCoinoshi</option> \
                </select> \
            </div> \
