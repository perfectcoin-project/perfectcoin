...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog, PerfectCoinBridge *bridge)
{
    if (!model) return;
    ...
    } else
    {
        ...
        int nDisplayUnit = BitcoinUnits::PC;
        ...
        l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01PC
