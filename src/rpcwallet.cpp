...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("perfectcoinxbalance", ValueFromAmount(pwalletMain->GetPerfectCoinXBalance())));
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new PerfectCoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current PerfectCoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <perfectcoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PerfectCoin address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <perfectcoinaddress> <amount> [comment] [comment-to] [narration]\n" // Exchanges use the comments internally...
            "sendtoaddress <perfectcoinaddress> <amount> [narration]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <perfectcoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <perfectcoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <perfectcoinaddress> [minconf=1]\n"
            "Returns the total amount received by <perfectcoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <toperfectcoinaddress> <amount> [minconf=1] [comment] [comment-to] [narration] \n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid PerfectCoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a PerfectCoin address or hex-encoded public key\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; PerfectCoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <perfectcoinaddress>\n"
            "Return information about <perfectcoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <perfectcoinpubkey>\n"
            "Return information about <perfectcoinpubkey>.");
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new PerfectCoin stealth address for receiving payments anonymously."
            + HelpRequiringPassphrase());
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    if (!sxAddr.SetEncoded(sEncoded))
        throw runtime_error("Invalid PerfectCoin stealth address.");
...
Value sendperfectcointoanon(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendperfectcointoanon <stealth_address> <amount> [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
    ...
    std::string sError;
    if (!pwalletMain->SendPerfectCoinToAnon(sxAddr, nAmount, sNarr, wtx, sError))
    {
        LogPrintf("SendPerfectCoinToAnon failed %s\n", sError.c_str());
        throw JSONRPCError(RPC_WALLET_ERROR, sError);
    };
    return wtx.GetHash().GetHex();
}
...
Value sendanontoperfectcoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendanontoperfectcoin <stealth_address> <amount> <ring_size> [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
    ...
    std::string sError;
    if (!pwalletMain->SendAnonToPerfectCoin(sxAddr, nAmount, nRingSize, sNarr, wtx, sError))
    {
        LogPrintf("SendAnonToPerfectCoin failed %s\n", sError.c_str());
        throw JSONRPCError(RPC_WALLET_ERROR, sError);
    };
    return wtx.GetHash().GetHex();
}
        ...
        for (it = listOrdered.begin(); it != listOrdered.end(); ++it)
        {
            ...
            for (uint32_t i = 0; i < pwtx->vin.size(); ++i)
            {
                ...
                if (pwtx->nVersion == ANON_TXN_VERSION
                    && txin.IsAnonInput())
                {
                    entry.push_back("perfectcoinx in");
                ...
                {
                    entry.push_back("perfectcoin in");
            ...
            for (unsigned int i = 0; i < pwtx->vout.size(); i++)
            {
                ...
                if (pwtx->nVersion == ANON_TXN_VERSION
                    && txout.IsAnonOutput())
                {
                    entry.push_back("perfectcoinx out");
                ...
                {
                    entry.push_back("perfectcoin out");
